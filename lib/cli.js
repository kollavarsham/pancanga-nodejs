...
var settingsQuestion = {
  ...
  choices : ['Latitude', 'Longitude', 'Bija', 'Go Back']
};
...
var getBanner = function () {
  return '\n' +
      '********* ' + chalk.green('Pancanga vers.3.13') + ' **********   ' + chalk.red('M. YANO and M. FUSHIMI') + '\n' +
      '----------------- perl version ------------- February 2004\n' +
      'This program is based on the Suryasiddhanta.\n' +
      'The constants of the text were modified  around AD 1000 by\n' +
      'the elements called \'bija\'. You can choose with or without bija\n' +
      'by entering \'' + chalk.blue('Bija') + '\' in \'' + chalk.blue('Settings') + '\' menu.\n\n' +
      ...
      '       because of adhimasa.) You should confirm it by menu \'List\'\n' +
      '  ' + chalk.green('List') + '  : To find the Indian date (in amanta) from the given modern\n' +
      ...
      '' + chalk.green('Verbose') + ' :   To get the further details than \'List\'\n' +
      '' + chalk.green('Settings') + ':   To set local \'Latitude\' & \'Longitude\' and \'Bija\'.\n' +
...
var displaySettings = function (settings) {
  ...
  console.log((settings.bija ? chalk.red('With') + ' Bija' : chalk.red('Without') + ' Bija') + '\n');
};
...
var longitudeLatitudeHandler = function (latitudeOrLongitude) {
  return function (answers) {
    var setter = 'set' + capitalize(latitudeOrLongitude);
    kollavarsham[setter](answers[latitudeOrLongitude]);
    showSettingsAndAskSettingsQuestion();
  };
};
...
var settingsQuestionHandler = function (answers) {
  switch (answers.settingMenu) {
    ...
    case 'Longitude':
      ...
      break;
    case 'Bija':
      kollavarsham.setBija(!kollavarsham.getSettings().bija);
      showSettingsAndAskSettingsQuestion();
      break;
    case 'Go Back':
...
var printListOutput = function (hinduDate) {
  console.log(sprintf('%4s %2s %2s %s|Saka %4s|V.S. %4s %17s %5s %2s %s',
      ...
      hinduDate.globals.adhimasa + hinduDate.globals.masa, hinduDate.globals.suklaKrsna, hinduDate.globals.tithiDay,
...
var printVerboseOutput = function (hinduDate, settings) {
  ...
  console.log(sprintf('  Pancanga based on Suryasiddhanta (sunrise) at latitude=%3s, longitude=%3s', settings.latitude, settings.longitude));
  console.log(sprintf('-------------------------------------------------------------------------------'));
  console.log(sprintf('      Nirayana  True Longitude   |  Lunar Month Names      |  Solar Month Names'));
  console.log('                 TODO: Port the sub named \'write_nirayana_longitude\'');
  console.log(sprintf('-------------------------------------------------------------------------------'));
  ...
  console.log(sprintf('             Jovian(North):%s |Jovian(South):%s',
      'TODO: Port the sub named \'get_Jovian_Year_name\'', 'TODO: Port the sub named \'get_Jovian_Year_name_south\''));  //  &get_Jovian_Year_name($YearKali))); and &get_Jovian_Year_name_south($YearKali)));
  ...
  console.log(sprintf(' %s %2s (fraction = 0.%s)', hinduDate.globals.suklaKrsna, hinduDate.globals.tithiDay,
  ...
  console.log(sprintf(' naksatra.... %s  /  karana...%s  /  yoga...%s', hinduDate.globals.naksatra,
      'TODO: Port the sub named \'get_karana_name\'', 'TODO: Port the sub named \'get_yoga_name\''));//  &get_karana_name($tithi))); and &get_yoga_name( $tslong, $tllong )));
...
var showSettingsAndAskSettingsQuestion = function () {
  displaySettings(kollavarsham.getSettings());
  inquirer.prompt([settingsQuestion], settingsQuestionHandler);
};

console.log(chalk.green.bold('\nWelcome to pancanga!') + chalk.gray('(node.js version)') + chalk.red(' by Arun N. Kutty and Hari Menon'));
